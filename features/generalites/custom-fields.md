...
Il se peut que certaines informations propres à votre secteur ne soit pas présente dans les champs natifs du registre des traitements, les exercices de droits...

Dastra vous permettra de créer des champs de formulaire personnalisés que vous pouvez très facilement ajouter pour enrichir vos formulaires.

Ces champs personnalisés seront également présents dans tous les rapports extraits de Dastra (Excel, Csv, Word, Html, PDF)

Voici un exemple de configuration de champs pour l'acteur :

<figure><img src="../../.gitbook/assets/image (8).png" alt=""><figcaption><p>Configuration des champs</p></figcaption></figure>

Voici le résultat dans le formulaire :
...
La quantité de champs personnalisés est limitée selon le nombre de plans que vous utilisez. Nous vous invitons à consulter l[a page des tarifs de l'application](https://www.dastra.eu/fr/pricing) pour plus d'informations à ce sujet
{% endhint %}
...
* Les champs personnalisés sont tous filtrables via le système de [filtres avancés](advanced-filters.md) à l'exception des champs comportant plusieurs réponses
...
Dastra vous permet de récupérer, modifier et créer via API toutes les valeurs des entités comprenant des champs personnalisés.
